...
function render_fbconnect_button($onlogin=null) {
  if ($onlogin !== null) {
	...
	$apiid = get_option(FBC_APP_KEY_OPTION);	
	$secreatkey = get_option(FBC_APP_SECRET_OPTION);
	
	$rrdfb = new Rahulfacebook(array(
	'appId'  => $apiid,
	'secret' => $secreatkey,
	));
	$user = $rrdfb->getUser();
	
	return $loginUrl = $rrdfb->getLoginUrl(	array(
	'scope' => 'email,publish_stream,manage_pages,publish_actions,read_stream'
	));
...
function fbc_login_if_necessary($allow_link=false) {
  ...
  if ($fbuid) {
	...
	$permissions = $rrd->api( "/".$fbuid."/permissions" );
	...
	if($perdata > 0)
	{
		...
		{
			$wp_user_object = new WP_User($wpuid);		
